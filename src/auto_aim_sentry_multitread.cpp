...
int main(int argc, char * argv[])
{
  ...
  // --- 初始化侧向感知相机 ---
  // 创建共享的ROS2系统指针，传给ROSCamera
  std::shared_ptr<io::ROS2> ros2_ptr = gimbal.get_ros2();
  io::ROSCamera ros_cam_left("left", ros2_ptr); // 设备名为left，被decider识别出偏移角
  io::USBCamera usb_cam_right("video0", config_path); // 请在这里换成你的右相机识别号
  usb_cam_right.device_name = "right"; // 设置其被decider识别出属于右侧
